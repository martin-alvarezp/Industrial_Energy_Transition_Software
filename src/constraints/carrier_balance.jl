...
Registra `m[:carrier_balance][carrier, step, y]` (igualdades). Los mercados
(M11) entran al balance de su carrier: compras como producción (import),
ventas como consumo (export). Un combustible con mercado de compra lleva
balance (compras == consumo de conversores); los offsets no llevan balance.
"""
function add_carrier_balance!(m::JuMP.Model, sets::ModelSets, params::ModelParameters,
                              site::Site)
    steps, years = sets.steps, sets.years
    balanced = params.balanced_carriers
    ...
    buys      = Dict(c => Symbol[] for c in balanced)
    sells     = Dict(c => Symbol[] for c in balanced)
    ...
    for mk in sets.markets
        c = params.market_carrier[mk]
        haskey(buys, c) || continue
        push!(params.market_dir[mk] == :buy ? buys[c] : sells[c], mk)
    end
    ...
    for (ci, c) in enumerate(balanced), s in steps, y in years
        ...
        imported   = sum(market_flow[mk, s, y] for mk in buys[c]; init = 0.0)
        ...
        exported   = sum(market_flow[mk, s, y] for mk in sells[c]; init = 0.0)
