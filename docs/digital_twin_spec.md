...
| **5 · Persistencia y modelo** ✅ | `save_site` (writer del contrato con round-trip garantizado por site_version física) + `GET /sites` + `PUT /sites/{name}` (+ layout.geojson; demo intocable; payload roto no se persiste); selector de sitio y "guardar como" en la tab; `storage_hours` como columna opcional del contrato CSV (default 4, retro-compatible); **valor residual** (`salvage_value` en ScenarioConfig): crédito capex·(vida−usados)/vida descontado al año N, con switch en el builder y columna `salvage_credit` en el desglose. Pendientes 🔮: availability por paso, overrides por año, creación de carriers en la UI | ✅ guardar "mi_planta" (e2e: zz_e2e_tmp), recargarla desde disco con layout restaurado y estado limpio, y correrla con valor residual activado (VAN baja y el crédito cuadra analíticamente) |
