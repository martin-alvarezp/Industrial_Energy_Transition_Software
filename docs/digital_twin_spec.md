...
| **2 · Tab Sitio: mapa + equipos** | react-leaflet + geoman + Nominatim; polígono límite; paleta y markers de equipos; drawer de equipo con TODOS los params del §4 (✅+📋); estado del twin en el front (carga de `GET /sites/demo`) | crear un "transformador de energía" nuevo (p.ej. caldera eléctrica 2) sobre el mapa y verlo en el estado serializado |
| **3 · Editores de series** | curvas de demanda por estación + growth + import CSV 8760→96; precios por carrier + escalaciones + export; factores de emisión; pesos temporales | editar la demanda térmica y ver el cambio reflejado en el payload validado |
| **4 · Run del twin** | [Validar] (dry-run contra validate_site vía un 400 amistoso o endpoint /validate) y [Ejecutar]: `site_payload` → cockpit/explorador muestran el resultado del twin; Excel del twin | flujo dirección→mapear→editar→optimizar→Excel completo con el motor real |
